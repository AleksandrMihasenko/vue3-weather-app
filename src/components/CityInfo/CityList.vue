...
import CityCard from "@/components/CityInfo/CityCard";
...
async function getCities() {
  ...
  savedCities.value.forEach(city => {
    requests.push(
      axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${city.coords.lat}&lon=${city.coords.lng}&appid=${openWeatherApiKey}`)
    )
  });
